...
AVFrame *PauseToggleVideo(VideoState *v, int pause)
{
	...
	if (!v->frame)
	{
		while (DecodeFrame(v, (int)((getTime() - v->startTime) / (1000.0 / 60.0))) != 1)
		{
		}
	}
...
int video_thread(void *data)
{
	...
	for (;;)
	{
		...
		if (v->pause_time)
		{
			v->startTime = getTime() - v->pause_time + v->pause_start_time;
		}
		...
		if (!DecodeFrame(v, (int)((getTime() - v->startTime) / (1000.0 / 60.0))))
		{
